...
class pixelArt(grid):  
    def drawGrid(self):
        ...
        #draw multiple thickness of lines
    ...
    def clearGrid(self): #pixels to the same color as the background color
...
class pixel():
    ...
    def getNeighbors(self, grid):
        ...
        i = self.col #current col value in the grid
        j = self.row #current row value in the grid
        rows = self.rows
        cols = self.cols
