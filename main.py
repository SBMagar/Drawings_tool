...
def fill(spot, grid, color, c):
   if spot.color != c:
      ...
      i = spot.col #col value in the grid
      j = spot.row #row value in the grid
...
def save(cols, rows, show, grid, path):
   if len(path) >= 4: #makes sure we have .txt at the end of our file selection
      if path[-4:] != '.txt':
         path = path + '.txt'
   else:
      path = path + '.txt'
...
def initalize(cols, rows, showGrid=False):
   ...
   buttons = ['Draw', 'Erase', 'Fill', 'Replace', 'Clear']
...
label1 = Label(window, text="Pixel Size: 20.0, 20.0")
var = IntVar()
    ...
    for event in ev:
        if event.type == pygame.QUIT:
            ...
            if pygame.display.get_caption()[0].count('*') > 0: 
               if messagebox.askyesno("Save Work?", "Would you like to save your work before closing?"):
        ...
        if pygame.mouse.get_pressed()[0]: #see clicked or dragged your mouse
            try:
                ...
                if pos[1] >= grid.height: # If the mouse is below the main drawing grid
                    if pos[0] >= tools.startx and pos[0] <= tools.startx + tools.width and pos[1] >= tools.starty and pos[1] <+ tools.starty + tools.height: #If the mouse ic clicking on the tools grid
                        ...
                        if clicked.text == 'Draw': #Draw tool  
                            color = (0,0,0)
                        elif clicked.text == 'Erase': #Erase tool
                            color = (255,255,255)
                        elif clicked.text == 'Fill':# Fill tool
                            doFill = True
                        elif clicked.text == 'Replace':# Replace tool
                            replace = True
                        elif clicked.text == 'Clear':# Clear grid tool
                            grid.clearGrid()
                            tools.drawGrid() #Redraw the grid so that we dont see the red highlight
                            buttons = ['Draw', 'Erase', 'Fill', 'Replace', 'Clear']
                            tools.setText(buttons)
